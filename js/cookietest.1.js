...
function showMap(position) {
	...
	if (typeof position === "object") {
		...
		// Store the location in a cookie for later
		setCookie("location.lat", position.coords.latitude, 365);
		consoleLog("cookie set: location.lat = " + getCookie("location.lat"));
		setCookie("location.lon", position.coords.longitude, 365);
		consoleLog("cookie set: location.lon = " + getCookie("location.lon"));
		...
		geocoder.geocode({"address": position}, function(results, status) {
			...
			if (status == google.maps.GeocoderStatus.OK) {
				...
				// Store the location in a cookie for later
				setCookie("location.lat", results[0].geometry.location.lat(), 365);
				consoleLog("cookie set: location.lat = " + getCookie("location.lat"));
				setCookie("location.lon", results[0].geometry.location.lng(), 365);
				consoleLog("cookie set: location.lon = " + getCookie("location.lon"));
